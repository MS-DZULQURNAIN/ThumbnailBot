...
async def thumb_change(bot, m):
    global thumb
    msg = await m.reply("`Sedang Mendownload..`", parse_mode='md')
    c_time = time.time()
    ...
    msg = await m.reply("`Sedang Mengupload..`", parse_mode='md')
    c_time = time.time()
    if m.document:
        ...
        await bot.send_video(chat_id=m.chat.id, video=file_dl_path, thumb=thumb, caption=("Add Thumbnail by @ThumbnailRobot"), progress=progress_for_pyrogram, progress_args=("Upload..", msg, c_time))
    await msg.delete()
